...
pub mod test_funcs {
        ...
        T: SignalTrainExt,
    {
        ...
        settings_signal.init_bf(in_.get(..len_sub_in).unwrap_or_default());
        assert_eq_pr!(
            settings_signal.signal_with_bf(in_.last().unwrap_or(&vec![0.0]),),
            eq,
        );
    }
    ...
    where
        ...
        T: SignalTrainExt,
    {
        assert_eq_pr!(settings_signal.signal(in_,), eq,);
    }
    ...
    {
        assert_eq_pr!(
            nz_coll::<Vec<f64>, f64, _>(&settings_signal.signals_vec(in_), 0.0),
